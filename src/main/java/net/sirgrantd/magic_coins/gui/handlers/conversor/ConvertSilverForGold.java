...
public record ConvertSilverForGold(int x, int y, int z) implements CustomPacketPayload {
    
    public static final Type<ConvertSilverForGold> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(MagicCoinsMod.MODID, "silver_for_gold_button"));
    public static final StreamCodec<RegistryFriendlyByteBuf, ConvertSilverForGold> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, ConvertSilverForGold message) -> {
        buffer.writeInt(message.x);
        buffer.writeInt(message.y);
        buffer.writeInt(message.z);
    }, (RegistryFriendlyByteBuf buffer) -> new ConvertSilverForGold(buffer.readInt(), buffer.readInt(), buffer.readInt()));
    ...
    public static void onCommonSetup(FMLCommonSetupEvent event) {
        MagicCoinsMod.addNetworkMessage(ConvertSilverForGold.TYPE, ConvertSilverForGold.STREAM_CODEC, ConvertSilverForGold::handleData);
    }
}
